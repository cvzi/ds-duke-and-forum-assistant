...
cuzi@openmail.cc
  ...
  Bugs:
  - won't work with different worlds/servers       Solved!
  - problems tribe barons/dukes                    Solved!
  - problems with premium                          Solved!
  - script is active in account sitting mode       Solved!  (script won't work in account sitting mode for now)
  - problems with shared forums                    Solved!

  ToDo:
  - large font-sizes                    Done!
  - large pics                          Done!
  - colours (if doable)                 Canceled!
  - multiple quotes (if doable)         In progress . . . .
  - Answer button in forum              Done! (on the type image)
  - disable duke functions              Done!
  - colour Picker                       Done!
...
if(getValue('search_active') == undefined || getValue('reset') === true)
  ...
  alert('DS Duke & Forum Assistant ist jetzt initialisiert');
...
function renderAllyMemberPage()
  ...
  if(ds_mitlieder_sortieren)
    ...
    var th = dom.n('th');
    th.setAttribute('title','Duke Info by example.com');
  ...
  if(ds_mitlieder_sortieren)
    ...
    var td = dom.n('td');
    td.appendChild(dom.text('Verwarnungen gesamt:'));
    ...
    var td = dom.n('td');
    td.appendChild(dom.text(String.fromCharCode('0216') + ' Durchschnitt:'));
    td.setAttribute('colspan',3);
    td.style.color = 'Gray';
    elist[i].appendChild(td);
...
function dialog_playerMenu(name)
  ...
  a_editNotes.appendChild(dom.text(String.fromCharCode('0187') + ' Notizen bearbeiten'));
  ...
  a_editColour.appendChild(dom.text(String.fromCharCode('0187') + ' Farbe ändern'));
  ...
  a_addWarning.appendChild(dom.text(String.fromCharCode('0187') + ' Verwarnen'));
  ...
  if(data[1] != 0)
    ...
    a_removeWarning.appendChild(dom.text(String.fromCharCode('0187') + ' Verwarnung aufheben'));
  ...
  frame.setTitle('Spielermenü - '+unescape(escape(name)));
  frame.setInstructions('Informationen zum Spieler '+unescape(escape(name)));
  frame.setButton('close','Schließen');
...
function dialog_playerMenu_editNotes(name)
  ...
  textarea.setAttribute('style','background:white url(http://example.com/images/gm_logo.png) bottom right no-repeat; ');
  textarea.appendChild(dom.text(data[2]));

  var frame = new chuanghu('dialog_playerMenu_editNotes');
  frame.setTitle('Notizen bearbeiten - '+unescape(escape(name)));
  frame.setInstructions('Bearbeite deine Notizen');
  frame.setButton('cancel','Abbrechen');
  frame.setButton('button','Speichern',dialog_playerMenu_editNotes_save);
  frame.addContent('table',
    {
    0:[
      dom.text('Notizen:')
...
function dialog_playerMenu_editColour(name)
  ...
  frame.setTitle('Farbe bearbeiten - '+unescape(escape(name)));
  frame.setInstructions('Bearbeite die im Forum dem Spieler zugewiesen wird');
  frame.setButton('cancel','Abbrechen');
  frame.setButton('button','Speichern',dialog_playerMenu_editColour_save);
  frame.addContent('table',
    {
    0:[
      dom.text('Aktuelle Farbe:'),
      dom.text(''),
        [
        dom.text(data[0]),
...
function dialog_playerMenu_removeWarning(name)
  ...
  frame.setTitle('Verwarnung aufheben - '+unescape(escape(name)));
  frame.setInstructions('Wähle die zu löschende Verwarnung aus');
  frame.setButton('cancel','Abbrechen');
  frame.setButton('button','Löschen',function() { dialog_playerMenu_removeWarning_save(name); });
  frame.addContent('table',
    {
    0:[
      dom.text('Verwarnungen:'),
...
function dialog_settings()
  ...
  var td = dom.n('td');
  td.appendChild(input);
  td.appendChild(dom.text('(innerhalb von *.die-staemme.de)'))
  tr.appendChild(td);
  content.appendChild(tr);
  // autoRedirect->
  ...
  var td = dom.n('td');
  td.appendChild(dom.text('Stammesführer aktiviert'));
  tr.appendChild(td);
  ...
  var td = dom.n('td');
  td.appendChild(input);
  td.appendChild(dom.text('(Memberliste)'))
  tr.appendChild(td);
  content.appendChild(tr);
  // memberlistSF->
  ...
  frame.setInstructions('Allgemeine Einstellungen');
  frame.setButton('cancel','Abbrechen');
  frame.setButton('button','Speichern',save_dialog_settings);
...
function dialog_export()
  ...
  a_json_download.appendChild(dom.text('JSON Download'));
  ...
  input_import_button.setAttribute('value','Importieren');
  ...
  frame.setTitle('Daten exportieren');
  frame.setInstructions('Diese Funktion ist nur für den Entwickler xD');
  frame.setButton('close','Schließen');
  ...
  frame.addContent('table',
    ...
    3:[
        [
        a_json_download,
        dom.text(' (ca. '+round((base64.length*0.75)/1024,1)+'kB)')
        ],
      input_import_button
      ]
...
function importJSONData(str)
  ...
  try
    ...
    text = 'Der eingegebene Code ist weder korrekter XML noch JSON Code.\n\n\nError description: \n\n' + text;
    alert(text);
    }
  }
...
function menu_extras()
  ...
  if(dom.id('menu_extras'))
    ...
    var a = dom.n('a');
    a.href = '#';
    a.appendChild(dom.text('Export/Import'));
    dom.addEvent(a,'click',function()
      {
      dom.id('menu_extras').style.display = 'none';
      dialog_export();
    ...
    dom.addEvent(a,'click',function()
      {
      dom.id('menu_extras').style.display = 'none';
      var q = confirm('Soll das Script wirklich auf Standardeinstellungen zurückgesetzt werden?');
      if(q)
        {
        var q = confirm('Letzte Möglichkeit das Zurücksetzen zu stoppen.\n\nZurücksetzen?');
        if(q)
          {
          setValue('reset',true);
          document.location.reload();
...
function clickedPlayerMenu(e,span)
  {
  var x = rel_left(span);
  var y = rel_top(span) + span.getSize().y;
  ...
  a.href = span.title;
  a.target = 'main';
  a.appendChild(dom.text( String.fromCharCode('0214') +'ffnen'));
  div.appendChild(a);
  ...
  a.appendChild(dom.text('Bearbeiten'));
  div.appendChild(a);
  ...
  a.target = 'main';
  a.appendChild(dom.text('Nachricht schreiben'));
  div.appendChild(a);

  var span = dom.n('span');
  span.setAttribute('style','display:block; font-size:x-small; padding-top:3px;');
  span.appendChild(document.createTextNode('Ranglisten:'));
  ...
  a.appendChild(document.createTextNode('Spieler'));
  div.appendChild(a);
  ...
  a.appendChild(document.createTextNode('Kontinent'));
  div.appendChild(a);
  ...
  a.appendChild(document.createTextNode('Besiegte Gegner als Angreifer'));
  div.appendChild(a);
  ...
  a.appendChild(document.createTextNode('Besiegte Gegner als Verteidiger'));
  div.appendChild(a);
  ...
  a.appendChild(document.createTextNode('Besiegte Gegner insgesamt'));
  div.appendChild(a);
...
function messageWindow(id)
  {
  var message_window = window.open('game.php?screen=mail&mode=new&player='+id+'&duke', 'Nachricht schreiben', 'width=480,height=250,left=100,top=200,menubar=no,resizable=yes,location=no');
  }
...
function adjustMessagePopup()
  ...
  dom.id('message').setAttribute('style','background:white url(http://example.com/images/gm_logo.png) bottom right no-repeat; ');
...
function addInviteLinks()
  ...
  dom.addEvent(a,'click',function()
    {
    if(confirm('Wirklich alle Einladungen zur'+String.fromCharCode('0252')+'ckziehen?'))
      returnAllInvitations();
    return false; });
  a.appendChild(dom.text('Alle Einladungen zurückziehen'));
...
function chuanghu(id)
  ...
  this.primaryKeys = eval({'buttons':0,'content':0});
...
function bar()
    ...
    rightfont.appendChild(dom.text('DS Duke & Forum Assistant'));
