...
function load()
  {
  if(url.indexOf('redir.php?url') != -1) // If we are on redirect page which ll appears if you klick on a external link
    {
...
function renderAllyMemberPage()
  ...
  for(var i = 0; i < ar.length; i++)
    ...
    var sub = ar[i].split('=');
    if(sub[0] && sub[1])
      names[unescape(sub[0])] = new Array(sub[1],sub[2]?sub[2]:0,sub[3]?sub[3]:'');
    }
  ...
  for(var i = 1, len = elist.length; len > i; i++)
    ...
    if(names[player])
      ...
      var td = dom.n('td');
      td.appendChild(dom.text(unescape(names[player][2])));
      elist[i].appendChild(td);
