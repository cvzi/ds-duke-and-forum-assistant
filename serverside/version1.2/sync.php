...
if($_GET['post']) {
	...
	$_SESSION['current_rid'] = $_GET['post'];
	...
	$_SESSION['current_rdata'] = '({requestready:true,finalUrl:"http://'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].'",responseText:\''.json_encode($output).'\'});';


	echo 'Form received';


} else if($_GET['get'] == $_SESSION['current_rid']) {


	echo $_GET['callback'].$_SESSION['current_rdata'];

	$_SESSION['current_rid'] = null;
	$_SESSION['current_rdata'] = null;
} else {
	echo $_GET['callback'].'({requestready:false});';
}
